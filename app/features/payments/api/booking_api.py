...
async def create_booking(
	...
	return await booking_service.create_booking_for_payment(user.id, payment_id, idempotency_key)
...
async def get_booking_for_payment(
	...
	return await booking_service.get_booking_by_payment(user.id, payment_id)

@router.post("/bookings/{booking_id}/update-calendar", status_code=204)
...
@router.post("/bookings/{booking_id}/send-confirmation", status_code=204)
